...
function workflowError(message: string) {
  ...
  if (/enqueue_internal_order_print|print_jobs|print_agent_tokens/i.test(message)) return "A impressão interna ainda não foi aplicada no Supabase. Execute a migration 026_internal_order_printing.sql.";
...
function printModeValue(value: unknown): PrintMode {
...
export default function InternalOrdersPage() {
  ...
  const [printMode, setPrintMode] = useState<PrintMode>("disabled");
  ...
  async function loadOperationData(branchId = selectedBranchId, silent = false) {
    ...
    const branchTenantId = branches.find((branch) => branch.id === branchId)?.tenant_id;
    const [parameterResult, tenantPrintParameterResult] = await Promise.all([
      supabase.from("store_parameters").select("parameter_key, parameter_value").eq("store_id", branchId).in("parameter_key", ["operation_flow", "production_release_mode", "internal_print_mode"]),
      branchTenantId
        ? supabase.from("tenant_parameters").select("parameter_value").eq("tenant_id", branchTenantId).eq("parameter_key", "internal_print_mode").maybeSingle()
    ...
    if (!parameterResult.error) {
      ...
      setPrintMode(printModeValue(parameters.get("internal_print_mode") ?? tenantPrintParameterResult.data?.parameter_value));
    }
  }
  ...
  async function queuePrint(order: InternalOrder) {
    if (!supabase || savingKey) return;
    setSavingKey(`print-${order.id}`);
    setError("");
    const { error: printError } = await supabase.rpc("enqueue_internal_order_print", { p_order_id: order.id, p_reason: "manual" });
    setSavingKey("");
    if (printError) {
      setError(workflowError(printError.message));
      return;
    }
    setNotice(`Comanda ${order.order_code} enviada para impressão.`);
  }
  ...
  const canPrintManual = printMode === "manual" || printMode === "manual_and_automatic";
...
function WaiterView({ sessions, orders, branch, flow, savingKey, canPrint, onPrint, onDeliverItem, onDeliverOrder, onDeliverSession, onRequestClosing }: { sessions: TableSession[]; orders: InternalOrder[]; branch: Branch | undefined; flow: OperationFlow; savingKey: string; canPrint: boolean; onPrint: (order: InternalOrder) => void; onDeliverItem: (orderId: string, itemId: string) => void; onDeliverOrder: (orderId: string) => void; onDeliverSession: (sessionId: string) => void; onRequestClosing: (sessionId: string) => void }) {
...
function KitchenView({ orders, flow, releaseMode, filter, onFilter, savingKey, canPrint, onPrint, onStart, onReady }: { orders: InternalOrder[]; flow: OperationFlow; releaseMode: ProductionReleaseMode; filter: "active" | "ready" | "all"; onFilter: (filter: "active" | "ready" | "all") => void; savingKey: string; canPrint: boolean; onPrint: (order: InternalOrder) => void; onStart: (orderId: string) => void; onReady: (orderId: string, itemId?: string) => void }) {
...
function CashierView({ sessions, orders, standaloneOrders, paymentBySession, onPaymentChange, savingKey, canPrint, onPrint, onRequestClosing, onReopen, onConfirmPayment, onRelease, onSettleStandalone }: { sessions: TableSession[]; orders: InternalOrder[]; standaloneOrders: InternalOrder[]; paymentBySession: Record<string, string>; onPaymentChange: (sessionId: string, method: string) => void; savingKey: string; canPrint: boolean; onPrint: (order: InternalOrder) => void; onRequestClosing: (sessionId: string) => void; onReopen: (sessionId: string) => void; onConfirmPayment: (sessionId: string, method: string) => void; onRelease: (sessionId: string) => void; onSettleStandalone: (order: InternalOrder) => void }) {
