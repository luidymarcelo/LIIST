...
test("keeps the growth surfaces present", async () => {
  ...
  assert.match(pageStyles, /\.direct-store-theme \.quantity-stepper button:active,[\s\S]*?background: var\(--primary, #111513\);[\s\S]*?transition-duration: 0s;/);
  ...
  assert.match(adminPage, /\.from\("tenants"\)\s*\.update\(identityData\)\s*\.eq\("id", tenant\.id\)/);
  ...
  assert.match(companyOwnerTenantReadSql, /company owners read own company/);
  assert.match(companyOwnerTenantReadSql, /on public\.tenants for select/);
  assert.match(companyOwnerTenantReadSql, /using \(public\.is_company_owner\(id\)\)/);
